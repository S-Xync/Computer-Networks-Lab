...
void* connection_handler(void* socket_desc){
  ...
  fflush(stdout);
  if(fp=fopen("tsserver.txt","r")){
    printf("Opened file tsserver.txt in reading mode.\n");
    while(1){
      int i=0;
      for(i=0;i<400;i++){
        ch=getc(fp);
      ...
      if(n==2){
        printf("Client Sent NO. Closing the connection\n");
        fflush(stdout);
        close(connection);
        return 0;
      }else if(n==3){
    ...
    printf("Unable to open the file tsserver.txt in reading mode\n");
    fflush(stdout);
    close(connection);
    return 0;
  }
